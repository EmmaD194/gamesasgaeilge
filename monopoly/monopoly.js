function Game() {
	...
	this.next = function() {
		if (!p.human && p.money < 0) {
			...
			if (p.money < 0) {
				popup("<p>" + " Tá " + p.name + " féimheach. Tabharfar a sócmhainní go léir do " + player[p.creditor].name + ".</p>", game.bankruptcy);
			} else {
				roll();
			}
	...
	var finalizeAuction = function() {
		...
		if (highestbid > 0) {
			...
			addAlert("Cheannaigh " + p.name  + " " + sq.name + " ar €" + highestbid + ".");
		}
	...
	this.auction = function() {
		...
		popup("<div style='font-weight: bold; font-size: 16px; margin-bottom: 10px;'>Ceant <span id='propertyname'></span></div><div>Tairiscint is airde = €<span id='highestbid'></span> (<span id='highestbidder'></span>)</div><div><span id='currentbidder'></span>, is é do sheal féin tairiscint a dhéanamh.</div<div><input id='bid' title='Enter amount to bid on " + s.name + ".' style='width: 291px;' /></div><div><input type='button' value='Tairiscint' onclick='game.auctionBid();' title='Submit your offer.' /><input type='button' value='Tharam!' title='Skip making an offer this time.' onclick='game.auctionPass();' /><input type='button' value='Scoir an cheant' title='Stop bids on " + s.name + ".' onclick='if (confirm(\"An bhfuil tú cinnte gur mhaith leat tairiscintí a stopadh ar an maoin seo?\")) game.auctionExit();' /></div>", "blank");
			...
			if (currentbidder == highestbidder) {
				...
				if (!p.human) {
					...
					if (bid === -1 || highestbid >= p.money) {
						...
						window.alert("D'fhág " + p.name + "an cheant.");
						continue;

					} else if (bid === 0) {
						window.alert("Níor chuir " + p.name + " tairiscint isteach.");
						continue;

					} else if (bid > 0) {
						this.auctionBid(bid);
						window.alert("Rinne " + p.name + " tairiscint €" + bid + ".");
						continue;
					}
					return;
				} else {
	...
	this.auctionBid = function(bid) {
		...
		if (bid === "" || bid === null) {
			document.getElementById("bid").value = "Iontráil tairiscint le do thoil.";
			document.getElementById("bid").style.color = "red";
		} else if (isNaN(bid)) {
			document.getElementById("bid").value = "Caithfidh uimhir a bheith i do thairiscint.";
			document.getElementById("bid").style.color = "red";
		} else {

			if (bid > player[currentbidder].money) {
				document.getElementById("bid").value = "Níl go leor airgid agat chun €" + bid + "a thairiscint.";
				document.getElementById("bid").style.color = "red";
				...
				document.getElementById("bid").value = "Caithfidh do thairiscint a bheith níos mó ná an tairiscint is airde. (€" + highestbid + ")";
				document.getElementById("bid").style.color = "red";
			}
		}
	};
	...
	var tradeMoneyOnChange = function(e) {
		...
		if (isNaN(amount)) {
			this.value = "Caithfidh uimhir a bheith sa luach seo.";
		...
		if (amount < 0) {
			this.value = "Caithfidh an luach seo a bheith níos mó ná 0.";
			this.style.color = "red";
			return false;
		}
	...
	var resetTrade = function(initiator, recipient, allowRecipientToBeChanged) {
		...
		for (var i = 0; i < 40; i++) {
			...
			if (currentSquare.owner === initiator.index) {
				...
				currentTableCellCheckbox.title = "Seiceáil an bosca seo chun " + currentSquare.name + " a áireamh sa trádáil.";
				...
				if (currentSquare.mortgage) {
					currentTableCell.title = "Faoi Mhorgáiste";
		...
		if (initiator.communityChestJailCard) {
			...
			currentTableCellCheckbox.title = "Seiceáil an bosca seo chun an Cárta Saor ó Phríosún a áireamh sa trádáil.";
			...
			currentTableCell.textContent = "Cárta Saor ó Phríosún";
		...
		if (initiatorSideTable.lastChild) {
			...
			initiatorProperty.textContent =  "Níl aon réadmhaoin ag " + initiator.name +  " le trádáil.";
		}

		if (recipientSideTable.lastChild) {
			recipientProperty.appendChild(recipientSideTable);
		} else {
			recipientProperty.textContent = "Níl aon réadmhaoin ag " + recipient.name + " le trádáil.";
		}
		...
		if (allowRecipientToBeChanged && pcount > 2) {
			...
			nameSelect.title = "Roghnaigh imreoir le trádáil leis.";
		} else {
	...
	this.acceptTrade = function(tradeObj) {
		if (isNaN(document.getElementById("trade-leftp-money").value)) {
			document.getElementById("trade-leftp-money").value = "Caithfidh uimhir a bheith sa luach seo.";
			document.getElementById("trade-leftp-money").style.color = "red";
			return false;
		}

		if (isNaN(document.getElementById("trade-rightp-money").value)) {
			document.getElementById("trade-rightp-money").value = "Caithfidh uimhir a bheith sa luach seo.";
		...
		if (money > 0 && money > initiator.money) {
			document.getElementById("trade-leftp-money").value = "Níl €" + money +  " ag " + initiator.name + ".";
			document.getElementById("trade-leftp-money").style.color = "red";
			return false;
		} else if (money < 0 && -money > recipient.money) {
			document.getElementById("trade-rightp-money").value = "Níl €" + (-money) +  " ag " + recipient.name + ".";
			document.getElementById("trade-rightp-money").style.color = "red";
			return false;
		}
		...
		if (isAPropertySelected === 0) {
			popup("<p>Caithfear maoin amháin nó níos mó a roghnú chun trádáil a dhéanamh.</p>");

			return false;
		}

		if (showAlerts && !confirm(initiator.name + ", an bhfuil tú cinnte gur mhaith leat an malartú seo a dhéanamh le " + recipient.name + "?")) {
			return false;
		}
		...
		for (var i = 0; i < 40; i++) {
			...
			if (tradeObj.getProperty(i) === 1) {
				...
				addAlert("Fuair " + recipient.name + square[i].name + " ó " + initiator.name + "."); 
			} else if (tradeObj.getProperty(i) === -1) {
				square[i].owner = initiator.index;
				addAlert("Fuair " + initiator.name + square[i].name + " ó " + recipient.name + ".");
			}
		...
		if (tradeObj.getCommunityChestJailCard() === 1) {
			...
			addAlert("Fuair " + recipient.name + " Cárta Saor ó Phríosún " + initiator.name + ".");
		} else if (tradeObj.getCommunityChestJailCard() === -1) {
			initiator.communityChestJailCard = true;
			recipient.communityChestJailCard = false;
			addAlert("Fuair " + initiator.name + "Cárta Saor ó Phríosún " + recipient.name + ".");
		}
		...
		if (money > 0) {
			...
			addAlert("Fuair " + recipient.name + " €" + money + " ó " + initiator.name + ".");
		} else if (money < 0) {
			money = -money;
			...
			addAlert("Fuair " + initiator.name + " €" + money + " ó " +  recipient.name + ".");
		}
	...
	this.proposeTrade = function() {
		...
		if (initiator.human && !confirm(initiator.name + ", an bhfuil tú cinnte gur mhaith leat an malartú seo a dhéanamh le " + recipient.name + "?")) {
			return false;
		}
		...
		if (recipient.human) {
			...
			addAlert("Chuir " + initiator.name + " tús le trádáil le "  + recipient.name + "."); 
			popup("<p>Chuir " + initiator.name + " tús le trádáil leat " + recipient.name + ". Féadfaidh tú glacadh leis an tairiscint, é a dhiúltú nó a mhodhnú.</p>"); 
			
		} else {
			var tradeResponse = recipient.AI.acceptTrade(tradeObj);

			if (tradeResponse === true) {
				popup("<p>Ghlac " + recipient.name + " le do thairiscint.</p>");
				this.acceptTrade(reversedTrade);
			} else if (tradeResponse === false) {
				popup("<p>Dhiúltaigh " + recipient.name + " do thairiscint.</p>");
				return;
			} else if (tradeResponse instanceof Trade) {
				popup("<p>Mhol " + recipient.name + " fritairiscint.</p>"); 
	...
	this.eliminatePlayer = function() {
		...
		if (pcount === 1) {
			...
			popup("<p>Maith thú! Bhuaigh " + player[1].name + " an cluiche!</p><div>");
	...
	this.bankruptcyUnmortgage = function() {
		...
		var HTML = "<p>" + player[p.creditor].name + ", is féidir leat aon cheann de na hairíonna seo a leanas a cheannach ar ais, saor ó ús, trí chliceáil orthu. Cliceáil OK nuair a bheidh sé críochnaithe.</p><table>";
		var price;
		...
		for (var i = 0; i < 40; i++) {
			...
			if (sq.owner == p.index && sq.mortgage) {
				...
				HTML += "' onmouseover='showdeed(" + i + ");' onmouseout='hidedeed();'></td><td class='propertycellname'><a href='javascript:void(0);' title='Unmortgage " + sq.name + " ar €" + price + ".' onclick='if (" + price + " <= player[" + p.creditor + "].money) {player[" + p.creditor + "].pay(" + price + ", 0); square[" + i + "].mortgage = false; addAlert(\"" + "Cheannaigh "+ player[p.creditor].name  + " " + sq.name + " ar ais ar €" + price + ".\");} this.parentElement.parentElement.style.display = \"none\";'>Unmortgage " + sq.name + " (€" + price + ")</a></td></tr>";
	...
	this.resign = function() {
		popup("<p>An bhfuil tú cinnte gur mhaith leat éirí as?</p>", game.bankruptcy, "Tá/Níl");
	};
	...
	this.bankruptcy = function() {
		...
		if (pcount === 2 || bankruptcyUnmortgageFee === 0 || p.creditor === 0) {
			...
			addAlert("D’íoc " + pcredit.name + " €" + bankruptcyUnmortgageFee + " ús ar na réadmhaoine morgáistithe a fuarthas ó " + p.name + ".");
			popup("<p>" + pcredit.name + ", caithfidh tú ús €" + bankruptcyUnmortgageFee + " a íoc ar na réadmhaoine morgáistithe a fuair tú ó " + p.name + ".</p>", function() {player[pcredit.index].pay(bankruptcyUnmortgageFee, 0); game.bankruptcyUnmortgage();});
		}
	};
...
function popup(HTML, action, option) {
	...
	// Yes/No
	if (option === "yes/no") {
		document.getElementById("popuptext").innerHTML += "<div><input type=\"button\" value=\"Tá\" id=\"popupyes\" /><input type=\"button\" value=\"Níl\" id=\"popupno\" /></div>";
...
function updateDice() {
	...
	if (document.images) {
		...
		element0.title = "Dice (" + die0 + " spots)";
		element1.title = "Dice (" + die1 + " spots)";
...
function updateOwned() {
	...
	for (var i = 0; i < 40; i++) {
		...
		if (sq.owner == turn) {
			...
			mortgagetext = "";
			if (sq.mortgage) {
				mortgagetext = "title='Faoi Mhorgáiste' style='color: grey;'";
			}

			housetext = "";
			if (sq.house >= 1 && sq.house <= 4) {
				for (var x = 1; x <= sq.house; x++) {
					housetext += "<img src='images/house.png' alt='' title='Teach' class='house' />";
				}
			} else if (sq.hotel) {
				housetext += "<img src='images/hotel.png' alt='' title='Óstán' class='hotel' />";
			}
...
function updateOption() {
	...
	if (checkedproperty < 0 || checkedproperty >= 40) {
		...
		$("#buildings").show();
		document.getElementById("buildings").innerHTML = "<img src='images/house.png' alt='' title='Teach' class='house' />:&nbsp;" + housesum + "&nbsp;&nbsp;<img src='images/hotel.png' alt='' title='Óstán' class='hotel' />:&nbsp;" + hotelsum;
	...
	if (sq.mortgage) {
		...
		document.getElementById("mortgagebutton").title = "Unmortgage " + sq.name + " ar €" + Math.round(sq.price * 0.6) + ".";
		$("#buyhousebutton").hide();
		$("#sellhousebutton").hide();

		allGroupUnmortgaged = false;
	} else {
		document.getElementById("mortgagebutton").value = "Morgáistigh (€" + (sq.price * 0.5) + ")";
		document.getElementById("mortgagebutton").title = "Morgáistigh " + sq.name + " ar €" + (sq.price * 0.5) + ".";
		...
		if (sq.groupNumber >= 3) {
			...
			buyhousebutton.value = "Ceannaigh teach (€" + sq.houseprice + ")";
			sellhousebutton.value = "Díol teach (€" + (sq.houseprice * 0.5) + ")";
			buyhousebutton.title = "Ceannaigh teach ar  €" + sq.houseprice;
			sellhousebutton.title = "Díol teach ar  €" + (sq.houseprice * 0.5);

			if (sq.house == 4) {
				buyhousebutton.value = "Ceannaigh óstán (€" + sq.houseprice + ")";
				buyhousebutton.title = "Ceannaigh óstán ar €" + sq.houseprice;
			}
			if (sq.hotel == 1) {
				$("#buyhousebutton").hide();
				sellhousebutton.value = "Díol óstán (€" + (sq.houseprice * 0.5) + ")";
				sellhousebutton.title = "Díol óstán ar €" + (sq.houseprice * 0.5);
			}
			...
			for (var i = 0; i < max; i++) {
				...
				if (s.owner !== sq.owner) {
					...
					buyhousebutton.title = "Sular féidir leat teach a cheannach, caithfidh réadmhaoin uile an ghrúpa dathanna seo a bheith agat.";
				} else {
			...
			if (!allGroupUnmortgaged) {
				buyhousebutton.disabled = true;
				buyhousebutton.title = "Sular féidir leat teach a cheannach, ní mór duit réadmhaoin uile an ghrúpa dathanna seo a 'unmortgage'.";
			}
			...
			if (sq.house > minhouse) {
				...
				if (sq.house == 1) {
					buyhousebutton.title = "Sular féidir leat teach eile a cheannach, caithfidh teach amháin a bheith ag réadmhaoin eile an ghrúpa dathanna seo.";
				} else if (sq.house == 4) {
					buyhousebutton.title = "Sular féidir leat óstán a cheannach, caithfidh ceithre theach a bheith ag réadmhaoin eile an ghrúpa dathanna seo.";
				} else {
					buyhousebutton.title = "Sular féidir leat teach a cheannach, caithfidh " + sq.house + " theach a bheith ag réadmhaoin eile an ghrúpa dathanna seo.";
				}
			}
			if (sq.house < maxhouse) {
				...
				if (sq.house == 1) {
					sellhousebutton.title = "Sular féidir leat teach a dhíol, caithfidh teach amháin a bheith ag réadmhaoin eile an ghrúpa dathanna seo.";
				} else {
					sellhousebutton.title = "Sular féidir leat teach a dhíol" + sq.house + " caithfidh trí theach a bheith ag réadmhaoin eile an ghrúpa dathanna seo.";
				}
			}
			...
			if (!allGroupUninproved) {
				document.getElementById("mortgagebutton").title = "Sular féidir maoin a mhorgáistiú, ní mór réadmhaoin uile a ghrúpa dathanna a bheith neamhfheabhsaithe.";
...
function chanceCommunityChest() {
	...
	if (p.position === 2 || p.position === 17 || p.position === 33) {
		...
		popup("<img src='images/chor.png' style='height: 50px; width: 53px; float: left; margin: 8px 8px 8px 0px;' /><div style='font-weight: bold; font-size: 16px; '>Cliabh Pobail:</div><div style='text-align: justify;'>" + communityChestCards[communityChestIndex].text + "</div>", function() {
	...
	// Chance
		...
		popup("<img src='images/chance_icon.png' style='height: 50px; width: 26px; float: left; margin: 8px 8px 8px 0px;' /><div style='font-weight: bold; font-size: 16px; '>Deis:</div><div style='text-align: justify;'>" + chanceCards[chanceIndex].text + "</div>", function() {
...
function addamount(amount, cause) {
	...
	addAlert("Fuair " + p.name + " €"+ amount + " ó " + cause + ".");
}
...
function subtractamount(amount, cause) {
	...
	addAlert("Chaill " + p.name + " €"+ amount + " ó " + cause + ".");
}

function goToJail() {
	var p = player[turn];
	addAlert("Cuireadh " + p.name + " go príosún.");
	document.getElementById("landed").innerHTML = "Tá tú i bpríosún";

	p.jail = true;
	doublecount = 0;

	document.getElementById("nextbutton").value = "Críochnaigh do sheal";
	document.getElementById("nextbutton").title = "End your turn and go to the next player";
...
function payeachplayer(amount, cause) {
	...
	addAlert("Chaill " + p.name + " €"+ total + " ó " + cause + ".");
}
...
function collectfromeachplayer(amount, cause) {
	...
	addAlert("Fuair " + p.name + " €"+ total + " ó " + cause + ".");
}
...
function advance(destination, pass) {
	...
	if (typeof pass === "number") {
		if (p.position < pass) {
			...
			p.position = pass;
			p.money += 200;
			addAlert("Bhailigh " + p.name + " €200 toisc go ndeachaigh sí thart ar GABH.");
		}
	}
...
function streetrepairs(houseprice, hotelprice) {
	...
	if (cost > 0) {
		...
		if (houseprice === 40) {
			addAlert("Chaill " + p.name + " €" + cost + " do Cliabh Pobail.");
		} else {
			addAlert("Chaill " + p.name + " €"+ cost + " do Deis.");
		}
	}
...
function payFifty() {
	...
	addAlert("D’íoc " + p.name + " an fhíneáil caoga euro chun éirí as an bpríosún.");
	updateMoney();
	updatePosition();
}
...
function useJailCard() {
	...
	addAlert("D'úsáid " + p.name + " 'Cárta Saor ó bPríosún'.");
	updateOwned();
	updatePosition();
}
	...
	if (p.money - sq.houseprice < 0) {
		...
		if (sq.house < 4) {
			if (houseSum >= 32) {
				...
				sq.house++;
				addAlert("Chuir " + p.name + " teach ar " + sq.name + ".");
			}
			...
			if (hotelSum >= 12) {
				...
				sq.house = 5;
				sq.hotel = 1;
				addAlert("Chuir " + p.name + " óstán ar " + sq.name + ".");
			}
		}
...
function sellHouse(index) {
	...
	if (sq.hotel === 1) {
		sq.hotel = 0;
		sq.house = 4;
		addAlert("Dhíol " + p.name + " an t-óstán ar " + sq.name + ".");
	} else {
		sq.house--;
		addAlert("Dhíol " +p.name + " an teach ar " + sq.name + ".");
	}
	...
	for (var x = 1; x <= pcount; x++) {
		...
		for (var i = 0; i < 40; i++) {
			...
			if (sq.owner == x) {
				...
				if (sq.mortgage) {
					mortgagetext = "title='Morgáistithe' style='color: grey;'";
				}
				...
				if (sq.house == 5) {
					housetext += "<span style='float: right; font-weight: bold;'>1&nbsp;x&nbsp;<img src='images/hotel.png' alt='' title='Óstán' class='hotel' style='float: none;' /></span>";
				} else if (sq.house > 0 && sq.house < 5) {
					housetext += "<span style='float: right; font-weight: bold;'>" + sq.house + "&nbsp;x&nbsp;<img src='images/house.png' alt='' title='Teach' class='house' style='float: none;' /></span>";
				}
		...
		if (p.communityChestJailCard) {
			...
			HTML += "<tr><td class='statscellcolor'></td><td class='statscellname'>Cárta Saor ó bPríosún</td></tr>";
		...
		if (!write) {
			HTML += "Níl aon réadmhaoin ag " + p.name;
		} else {
			HTML += "</table>";
		}
...
function buy() {
	...
	if (p.money >= cost) {
		...
		property.owner = turn;
		updateMoney();
		addAlert("Cheannaigh " + p.name + " " + property.name + " ar " + property.pricetext + ".");
		...
		popup("<p>" + p.name + ", teastaíonn €" + (property.price - p.money) + " eile uait chun " + property.name + " a cheannach.</p>");
	}
}
...
function mortgage(index) {
	...
	document.getElementById("mortgagebutton").value = "Unmortgage ar €" + unmortgagePrice;
	document.getElementById("mortgagebutton").title = "Unmortgage " + sq.name + " ar €" + unmortgagePrice + ".";

	addAlert("Chuir " + p.name + " morgáiste ar " + sq.name + " ar €" + mortgagePrice + ".");
	updateOwned();
	updateMoney();
...
function unmortgage(index) {
	...
	sq.mortgage = false;
	document.getElementById("mortgagebutton").value = "Morgáistigh ar €" + mortgagePrice;
	document.getElementById("mortgagebutton").title = "Morgáistigh " + sq.name + " ar €" + mortgagePrice + ".";

	addAlert(p.name + " unmortgaged " + sq.name + " ar €" + unmortgagePrice + ".");
	updateOwned();
	return true;
}
...
function land(increasedRent) {
	...
	$("#landed").show();
	document.getElementById("landed").innerHTML = "Thuirling tú ar " + s.name + ".";
	s.landcount++;
	addAlert("Thuirling " + p.name + " ar " + s.name + ".");
	...
	if (s.price !== 0 && s.owner === 0) {
		...
		if (!p.human) {
			...
			document.getElementById("landed").innerHTML = "<div>Thuirling tú ar <a href='javascript:void(0);' onmouseover='showdeed(" + p.position + ");' onmouseout='hidedeed();' class='statscellcolor'>" + s.name + "</a>.<input type='button' onclick='buy();' value='Ceannaigh (€" + s.price + ")' title='Buy " + s.name  + " " + s.pricetext + ".'/></div>";
		}
	...
	if (s.owner !== 0 && s.owner != turn && !s.mortgage) {
		...
		addAlert("D'íoc " + p.name + " cíos €" + rent + " le " + player[s.owner].name + ".");
		p.pay(rent, s.owner);
		player[s.owner].money += rent;

		document.getElementById("landed").innerHTML = "Thuirling tú ar " + s.name + ". " + "Bhailigh " + player[s.owner].name + " cíos €" + rent + ".";
	} else if (s.owner > 0 && s.owner != turn && s.mortgage) {
		document.getElementById("landed").innerHTML = "Thuirling tú ar " + s.name + ". Tá an réadmhaoin seo morgáistithe. Níor bailíodh aon chíos.";
	}
	...
	if (p.position === 30) {
		...
		if (p.human) {
			popup("<div>Téigh go dtí an príosún. Téigh go díreach chuig an bpríosún. Ná téigh thart ar \"GABH \". Ná bailigh € 200.</div>", goToJail());
		} else {
			goToJail();
		}
...
function roll() {
	...
	if (die1 == die2) {
		addAlert("Rollaigh " + p.name + " dúbailtí " + (die1 + die2));
	} else {
		addAlert("Rollaigh " + p.name + " " + (die1 + die2) + ".");
	}
	...
	if (die1 == die2 && !p.jail) {
		...
		if (doublecount < 3) {
			document.getElementById("nextbutton").value = "Rollaigh arís";
			document.getElementById("nextbutton").title = "Chaith tú dúbailtí. Rollaigh arís.";

		// If player rolls doubles three times in a row, send him to jail
		} else if (doublecount === 3) {
			p.jail = true;
			doublecount = 0;
			addAlert("Rolladh " + p.name + " dubailtí trí huaire as a chéile."); 
			updateMoney();


			if (p.human) {
				popup("Rolladh tú dubailtí trí huaire as a chéile. Téigh go dtí an príosún.", goToJail);
			} else {
				goToJail();
			}
	...
	if (p.jail === true) {
		...
		if (die1 == die2) {
			...
			addAlert("Rollaigh " + p.name + " dubailtí chun éirí as an bpríosún");
			...
			if (p.jailroll === 3) {
				...
				if (p.human) {
					popup("<p>Ní mór duit an fhíneáil €50 a íoc.</p>", function() {
						payFifty();
						payFifty();
		...
		if (p.position >= 40) {
			p.position -= 40;
			p.money += 200;
			addAlert("Bhailigh " + p.name + " toisc go ndeachaigh sí thart ar GABH");
		}
...
function play() {
	...
	document.getElementById("nextbutton").value = "Rollaigh na dísle";
	...
	if (p.jail) {
		$("#landed").show();
		document.getElementById("landed").innerHTML = "Tá tú i bpríosún.<input type='button' title='Íoc €50 chun éirí as an bpríosún láithreach.' value='Íoc €50' onclick='payFifty();' />";

		if (p.communityChestJailCard || p.chanceJailCard) {
			document.getElementById("landed").innerHTML += "<input type='button' id='gojfbutton' title='Úsáid &quot;Cárta Saor ó Príosún&quot;.' onclick='useJailCard();' value='Bain úsáid an gcárta' />";
		}

		document.getElementById("nextbutton").title = "Rollaigh na dísle. Má chaitheann tú dúbailtí, gheobhaidh tú amach as an bpríosún é.";

		if (p.jailroll === 0)
			addAlert("Seo an chéad sheal ag " + p.name + " sa phríosún.");
		else if (p.jailroll === 1)
			addAlert("Seo an dara seal ag " + p.name + " sa phríosún.");
		else if (p.jailroll === 2) {
			document.getElementById("landed").innerHTML += "<div>NOTE: Mura gcaitheann tú dúbailtí tar éis an rolla seo, caithfidh tú an fhíneáil €50 a íoc.</div>";
...
window.onload = function() {
	...
	document.getElementById("enlarge-wrap").innerHTML += "<div id='enlarge40' class='enlarge'><div id='enlarge40color' class='enlarge-color'></div><br /><div id='enlarge40name' class='enlarge-name'>Jail</div><br /><div id='enlarge40price' class='enlarge-price'><img src='images/injail_icon.png' height='80' width='80' alt='' style='position: relative; top: -20px;' /></div><br /><div id='enlarge40token' class='enlarge-token'></div></div>";
	...
	$("#mortgagebutton").click(function() {
		...
		if (s.mortgage) {
			if (player[s.owner].money < Math.round(s.price * 0.6)) {
				popup("<p>Teastaíonn €" + (Math.round(s.price * 0.6) - player[s.owner].money) + " níos mó uait chun an réadmhaoin seo a unmortgage " + s.name + ".</p>");

			} else {
				popup("<p>" + player[s.owner].name + ", an bhfuil tú cinnte gur mhaith leat " + s.name + " a unmortgage ar  €" + Math.round(s.price * 0.6) + "?</p>", function() {
					unmortgage(checkedProperty);
				}, "Yes/No");
			}
		} else {
			popup("<p>" + player[s.owner].name + ", an bhfuil tú cinnte gur mian leat morgáiste " + s.name + " ar €" + Math.round(s.price * 0.5) + "?</p>", function() {
	...
	$("#buyhousebutton").on("click", function() {
		...
		if (p.money < s.houseprice) {
			if (s.house === 4) {
				popup("<p>Teastaíonn €" + (s.houseprice - player[s.owner].money) + " níos mó uait chun óstán a cheannach do " + s.name + ".</p>");
				return;
			} else {
				popup("<p>Teastaíonn  €" + (s.houseprice - player[s.owner].money) + " níos mó uait chun óstán a cheannach do " + s.name + ".</p>");
				return;
		...
		if (s.house < 4 && houseSum >= 32) {
			popup("<p>Tá gach teach faoi úinéireacht. Caithfidh tú fanacht go dtí go mbeidh ceann ar fáil.</p>");
			return;
		} else if (s.house === 4 && hotelSum >= 12) {
			popup("<p>Tá gach óstán faoi úinéireacht. Caithfidh tú fanacht go dtí go mbeidh ceann ar fáil.</p>");
			return;
		}
