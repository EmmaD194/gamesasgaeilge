...
$(document).ready(function(){
	...
	var blueTiles = [1,3,9,14,18,20,21,25,27,40,41,47,54,58,60,61,63,69];
	var yellowTiles = [2,8,10,11,13,17,24,26,30,31,44,50,51,53,57,62,68,70];
	...
	for(var x = 1; x<71; x++){
		...
		for(var y = 0; y<blueTiles.length; y++){
			if(x == blueTiles[y]) {
				boardhtml += ' blue droppable';
				
			}
			
		}
		for(var y = 0; y<yellowTiles.length; y++){
			if(x == yellowTiles[y]) {
				boardhtml += ' yellow droppable';
			}
		...
		{
			boardhtml += '<div class="startEnd">Tús</div>';
		} else if (x==47)
		{
			boardhtml += '<div class="startEnd">Deireadh</div>';
		}
		boardhtml += '</div>';
	}
